...
作为最后一道防线，我们应该专注于完整的包裹式隔离和用户授权之间的平衡。这里说“平衡”，是因为过度麻烦的授权流程会推动用户主动打破安全边界。
...
1. 预知。组合/特例越多，漏洞越多。不要尝试预言所有的任务组合。你无法提前预测完成任务需要哪些文件或数据，所以权限应该动态申请、动态授权。
2. 相信“安全的”提示词。不要只在 MCP Tool / skill 描述里写权限规则。提示词规则不能替代运行时强制隔离。
