...
> 人类的问题是，做事的人总是太年轻。AI 行业正在重新发现《控制论》（Wiener, 1948）[1]。
>
> 不要想象“智能”，也不要以为机械。
...
这也解释了为什么行业里会有“提示词迁移”问题：好的提示方法往往可以跨模型复用，具体措辞却未必能原样迁移。同一段文字进入不同模型后，展开的生成路径并不相同：一个模型上的有效表达，换到另一个模型上可能只得到次优结果。一项自然语言提示研究发现，仅仅改写短语或调换组成部分的顺序，就可能大幅改变结果；这种顺序依赖出现在多个二水平部分因子设计实例，以及不同的 GPT 和 Gemini 模型中（[Ying et al., 2026](https://arxiv.org/abs/2607.05537)）[2]。所以，如果一个简单的提示词在多个模型上都能取得相近效果，更有理由认为它抓住了通用的方法；复杂措辞若只在某个模型上带来额外收益，则更可能依赖该模型特有的响应偏好。

#### 少写“不要”，多写目标

[Anthropic 2026 年关于语言模型 global workspace 的研究](https://transformer-circuits.pub/2026/workspace/)[3]让这个经验有了更具体的参照。论文的总体结构分析使用 J-lens 读取模型内部**可言说的表示**，观察某些表示如何跨 token 位置广播、如何受任务指令调节，并由此讨论语言模型中类似 global workspace 的结构。prompt context 承载输入，J-space 是 J-lens 读取表示的空间，global workspace 则描述论文观察到的广播结构。
...
关于系统提示词、最新输入和工具结果为什么重要，我在 [AI Agent Privacy and Protection](https://xzos.net/blog/ai-agent-privacy-and-protection/)[4] 里也从安全角度讨论过。
...
> [Remember When It Matters: Proactive Memory Agent for Long-Horizon Agents](https://arxiv.org/abs/2607.08716)[5] 也采用了相近的架构：执行任务的 Agent 保持不变，独立运行的 memory Agent 从近期轨迹中更新结构化记忆，并判断何时将压缩后的相关记忆注入其上下文，以缓解长轨迹中的“行为状态衰减”。
