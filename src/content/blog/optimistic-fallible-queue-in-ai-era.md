...
title: "AI 时代的乐观可失败队列"
pubDate: "2026-02-04"
description: "为 MCP 官方 Memory Server 修复并发写入问题的思考：当 AI 成为用户，软件的本质变成了提供一种百分百的概率。"
author: "xz-dev"
category: "AI"
...
一开始我自己实现了：先写临时文件，再 rename。但很快发现文件权限、并发临时文件命名等边界情况太多，于是改用第三方库（write-file-atomic）。
...
MCP 工具可以不存在，但是 AI 还是最终会用“概率”“发明”一个不稳定的 MCP，所以 AI 时代，软件的本质是提供一种百分百的概率，是一定成功和一定失败的机器逻辑（是不是突然想到 Claude Skill 了）。

因此 AI 原生软件的优雅便是充分的利用概率。

因此，线程锁，和导致的实际上的队列现象，便可以是乐观的。但不能极度乐观，也就是操作失败数量远远大于成功的（这往往意味着根本上失败的技术选型）。
